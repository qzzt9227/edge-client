...
public final class DropPointModule extends Module {
	...
	public DropPointModule(MessageBoxApi messages) {
		...
		defaultColor = colors("default", 0x33BFFF, 0.35);
		dangerColor = colors("danger", 0xFF1A1A, 0.45);
		safeColor = colors("safe", 0x1AFF26, 0.4);
		scaffoldSneakingColor = colors("scaffold_sneaking", 0x1AFF26, 0.4);
		scaffoldNeedsSneakingColor = colors("scaffold_needs_sneaking", 0xFFBF0D, 0.45);
	...
	private DropPointColorSettings colors(
		...
		return new DropPointColorSettings(this::setting, idPrefix, defaultRgb, opacity);
	}
}
