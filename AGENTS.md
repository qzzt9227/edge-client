...
- 严格遵循多语言分离规则：中文语言下只显示纯中文名称，不得在后面用括号添加英文名称；仅在游戏或客户端设置为英文时显示对应英文名称（详见后文“语言与多语言本地化约定”）。
...
1. **中文语言环境 (`zh_cn.json`)**：
   - 只能显示纯正中文名称与描述。
   - **严禁**在中文名称后使用括号等形式标注英文（例如：禁止出现 `粒子效果 (Particles)`、`静态实体与界面 (Static Entities & UI)`，必须为 `粒子效果`、`静态实体与界面`）。
2. **英文语言环境 (`en_us.json`)**：
   - 当游戏或客户端语言设置为英文时，显示完整的对应英文名称与描述（例如 `Particles`、`Static Entities & UI`）。
3. **职责分离**：
   - 各语言文件各司其职，不在中文语言包中混合双语标注，保证界面视觉统一与整洁。后续新增或修改任何模块、功能、设置项与选项翻译时均必须严格遵守此规则。
